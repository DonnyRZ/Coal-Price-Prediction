...
## 📊 模型表现 (Performance Evaluation)

下图展示了模型在 2023-2025 年全量数据上的回测表现（包含训练集与测试集）。

![Model Performance](assets/model_performance_report.png)

* **图表说明**：
    * **上图 (Equity Curve)**: 红色实线为 **AI 策略净值**，灰色虚线为 **买入持有 (Buy & Hold)** 基准。蓝色虚线左侧为训练集，右侧为未见过的测试集。
    * **下图 (Log Returns)**: 展示了预测收益率 (蓝色) 与真实收益率 (黑色) 的拟合程度。
    * **指标**: 图中方框内标注了 **DA (方向准确率)** 和 **RMSE (均方根误差)**，用于量化评估模型在不同阶段的稳定性。
---
